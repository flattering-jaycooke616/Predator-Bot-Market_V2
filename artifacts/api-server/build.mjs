...
async function buildAll() {
  ...
  await mkdir(path.resolve(artifactDir, "api"), { recursive: true });

  // Main server build (with listen)
  await esbuild({
  ...
  // Vercel serverless build (app only, no listen)
  const vercelApiDir = path.resolve(artifactDir, "api-vercel");
  ...
  // Move the bundled app to api/index.js for Vercel
  const { rename } = await import("node:fs/promises");
  await rename(path.resolve(vercelApiDir, "app.js"), path.resolve(artifactDir, "api/index.js"));
  await rm(vercelApiDir, { recursive: true, force: true });
}
